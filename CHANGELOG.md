...
### Added
- 🗺️ **WGS84 ↔ HK1980 座標轉換模組** (`js/coords.js`)：使用 `proj4` 庫進行 EPSG:4326 ↔ EPSG:2326 (HK1980) 雙向座標轉換，符合香港地政總署測量標準
- 樹木表單新增 **Easting / Northing** 欄位（HK1980 網格座標），支援直接輸入香港測量座標
- **座標系統切換按鈕**：WGS84 / HK1980 一鍵切換顯示模式，自動隱藏/顯示對應欄位
- **雙向自動同步**：輸入 WGS84 經緯度自動計算 HK1980 網格值，反之亦然（`syncCoords`）
- **`switchCoordMode`** 函數：整合座標模式切換邏輯與 UI 更新
- CDN 資源映射表（`config.js`）新增 `proj4@2.11.0`
- `trees` 資料表新增 `easting`、`northing` 欄位
- 地圖模組 (`map.js`) 整合 `TreeApp.coords` 進行座標讀取與轉換

### Changed
- `gps.js` GPS 定位回填同時計算 WGS84 與 HK1980 雙套座標
- `index.html` 引入 `js/coords.js` 模組腳本

### Security
- 座標轉換完全在前端執行，不暴露位置資料至第三方服務
